...
DP_SPONSOR varchar2(200),
DP_VIEWTIME	varchar2(500),
DP_VIEWCHARGE varchar2(500),
DP_ART_PART	varchar2(300),
DP_ART_CNT varchar2(80),
...
UPDATE exhibitionData SET dp_viewcharge = '무료' WHERE dp_viewcharge is null ;

UPDATE exhibitionData SET dp_viewcharge = '유료' WHERE dp_seq = 707;
...
ALTER TABLE exhibitionData ADD dp_charge_adult number(5) DEFAULT 0 NOT NULL;
ALTER TABLE exhibitionData ADD dp_charge_teen number(5) DEFAULT 0 NOT NULL;
ALTER TABLE exhibitionData ADD dp_charge_child number(5) DEFAULT 0 NOT NULL;

-- 5. 요금정보가 유료인 고유번호 707 전시회에 성인, 청소년, 아동 요금의 데이터를 각각 추가
UPDATE exhibitionData SET dp_charge_adult = 17000 WHERE dp_seq = 707;
UPDATE exhibitionData SET dp_charge_teen = 15000 WHERE dp_seq = 707;
UPDATE exhibitionData SET dp_charge_child = 12000 WHERE dp_seq = 707;

-- 6. 2023-04-02 전시종료일자보다 늦는 전시회 정보 (우리가 사용하는 데이터) 조회.. 잘 들어왔는지 
